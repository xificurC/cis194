...
takeNth :: [a] -> Int -> [a]
takeNth a i = case drop (i-1) a of
                [] -> []
                (x:xs) -> x : takeNth xs i

-- skips - given a list returns a list of lists 
-- that contains every [1..n]th element of the inital list
skips :: [a] -> [[a]]
skips lst = map (takeNth lst) [1..(length lst)]
...
histogram lst = unlines . transpose $ map
                (\x -> buildString
                       (M.findWithDefault 0 x fMap)
                       maxMap
                       (fromInteger x))
                [0..9]
